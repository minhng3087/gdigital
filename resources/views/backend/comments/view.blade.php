						    ...
						    <div class="box-header with-border">
						        <div class="user-block" >
						            <span class="username" style="margin-left: 0px">
										<a href="{{ route('home.single.product', $data->slug) }}" target="_blank">
										{{ Request::segment(5) == 'product' ? 'Sản phẩm' : 'Bài viết' }} :  {{ $data->name }}
									</a></span>
						        </div>
						    </div>
						    <div class="box-footer box-comments">
						    	@if(Request::segment(5) == 'product')
									    ...
									    <div class="box-comment">
									        <div class="comment-text">
									            <span class="username"> {{ $item->type != 1 ? @$item->Customers->name.' - '.@$item->Customers->email.' - '.@$item->Customers->phone : 'Administrator' }} 
									            	<span class="text-muted pull-right">{{ $item->created_at->format('d/m/Y') }}</span>
									            	@if ($item->status != 1)
